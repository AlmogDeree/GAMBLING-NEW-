...
export default function HomePage() {
  ...
  const fetchUserData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://192.248.151.61:5000/api/user', {
  ...
  const fetchRecentWins = async () => {
    try {
      const response = await axios.get('http://192.248.151.61:5000/api/recent-wins');
  ...
  const fetchHotColdNumbers = async () => {
    try {
      const response = await axios.get(`http://192.248.151.61:5000/api/number-stats?range=${range}`);
  ...
  const fetchLeaderboard = async () => {
    try {
      const response = await axios.get('http://192.248.151.61:5000/api/leaderboard');
  ...
  const startGame = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post('http://192.248.151.61:5000/api/play', {
        selectedNumbers,
        range,
        betAmount
      }, {
