...
export default function AdminPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
    ...
    <div className="min-h-screen text-white bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-xl font-bold text-white">{t.adminPanel}</div>
                <div className="text-sm text-gray-300">{t.hello}, {user?.username}</div>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex gap-1">
            {[
              ...
              { id: 'bets', name: t.recentBets, icon: Activity }
            ].map((tab) => (
              <button
                key={tab.id}
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              >
                ...
                <div className="text-sm text-gray-300">{t.totalUsers}</div>
                <div className="text-xs text-blue-300 mt-1">{stats.activeUsers} {t.weeklyActive}</div>
              </motion.div>
              ...
              >
                ...
                <div className="text-sm text-gray-300">{t.totalBets}</div>
                <div className="text-xs text-green-300 mt-1">{stats.betsToday} {t.todayBets}</div>
              </motion.div>
              ...
              >
                ...
                <div className="text-sm text-gray-300">{t.winRate}</div>
                <div className="text-xs text-yellow-300 mt-1">{stats.totalWins} {t.totalWins}</div>
              </motion.div>
              ...
              >
                ...
                <div className="text-sm text-gray-300">{t.revenue}</div>
                <div className="text-xs text-green-300 mt-1">{t.houseEdge}</div>
              </motion.div>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-black/20 border-b border-white/10">
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.username}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.balance}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.vipLevel}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.statistics}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.profit}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.actions}</th>
                    </tr>
                  </thead>
                    ...
                    {users.map((userItem) => (
                      <tr key={userItem._id} className="hover:bg-white/5">
                        ...
                        <td className="px-6 py-4 text-sm">
                          <div className="text-white">{userItem.stats.totalBets} {t.bets}</div>
                          <div className="text-green-400">{userItem.stats.totalWins} {t.totalWins}</div>
                          <div className="text-yellow-400">{userItem.stats.winRate}% {t.winRate}</div>
                        </td>
                        <td className="px-6 py-4">
        ...
        {activeTab === 'bets' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-white">{t.recentBets}</h2>
              <button
                onClick={fetchAdminData}
                className="bg-blue-600/80 border border-blue-400 rounded-lg px-4 py-2 hover:bg-blue-600/90 transition-colors flex items-center gap-2 text-white"
              >
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-black/20 border-b border-white/10">
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.username}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.yourNumber}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.systemNumbers}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.betAmount}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.result}</th>
                      <th className="px-6 py-4 text-left text-sm font-medium text-gray-300 uppercase tracking-wider">{t.date}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/10">
                    {recentBets.map((bet) => (
