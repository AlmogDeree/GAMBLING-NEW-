...
app.post('/api/play', auth, betLimiter, async (req, res) => {
  try {
    ...
    // Generate system numbers (single spin)
    const systemNumbers = [];
    const numSystemNumbers = Math.min(3, Math.max(1, Math.floor(selectedNumbers.length / 2)));
    
    while (systemNumbers.length < numSystemNumbers) {
      const randomNum = Math.floor(Math.random() * range) + 1;
      if (!systemNumbers.includes(randomNum)) {
        systemNumbers.push(randomNum);
      }
    }
